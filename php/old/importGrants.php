<?php
	...
	$file             = "./University_of_Notre_Dame/NotreDameFellowships_Oct9_2013.json";
	...
	if ( file_exists($file) === true ){
	    $grants = json_decode(file_get_contents($file),true);
	    $grants = $grants["data"];
	    // Open the SQL
	    $con = mysqli_connect("localhost","svetlana","vH8ymo=nhwM6","svetlana_Total");
		if (mysqli_connect_errno($con)){
		...
		for ( $i = 0, $n = count($grants); $i < $n; $i++ ){
    		...
    		if ( empty($grant["name"] ) === false ){
    		    $name = $grant["name"][0];
        		// $name = utf8_decode(makeProperName($name)); This was for advisors
    		    if ( in_array($grant["name"][0],$exceptions) === false ){
        		    $sqlAction = "INSERT";
        		    // Get the name and check to see if the name exists in the database
        		    $nameCheck = runSQL("SELECT `University` FROM `Grant` WHERE `Title`='".mysqli_real_escape_string($con,trim($name))."'",$con);
    		        ...
    		        if ( $grant["description"] != null ){
    		            $description = implode("<br/><br/>",$grant["description"]);
    		        }
    		        // Remove iframes from the description
    		        if ( stripos($description,"<iframe") !== false ){
    		            $begin = stripos($description,"<iframe");
    		            $iframeData = substr($description,$begin);
    		            $iframeData = substr($iframeData,0,stripos($iframeData,"</iframe>"));
    		            $description = str_replace($iframeData,"",$description);
    		        }
        		    ...
        		    if ( $sqlAction == "INSERT" ){
        		        ...
        		        insertCourse($name,$university,$description,$con);
        		    }
        		    else if ( $sqlAction === "UPDATE" ){
        		        updateCourse($name,$university,$description,$con);
        		    }
    		    }
    		}
		}
	...
	function updateCourse($name,$university,$description,$connection_var){
        ...
        $select = "SELECT * FROM `Grant` WHERE `Title`='$name' AND `University`='$university'";
        $update = "UPDATE `Grant` SET `Description`='$description', `University`='$university' WHERE `Title`='$name' AND `University`='$university'";
    ...
    function insertCourse($name,$university,$description,$connection_var){
        $name        = mysqli_real_escape_string($connection_var,$name);
        $description = mysqli_real_escape_string($connection_var,$description);
        $university  = mysqli_real_escape_string($connection_var,$university);

        $insert  = "INSERT INTO `Grant` (`Title`,`University`,`Description`) VALUES (";
        $insert .= "'$name','$university','$description')";
